using Microsoft.EntityFrameworkCore;

namespace PPCore.Models
{
    public partial class PalangPanyaDBContext : DbContext
    {
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<course_grade>(entity =>
            {
                entity.HasKey(e => e.cgrade_code);
                entity.Property(e => e.cgrade_code)
                    .HasMaxLength(1)
                    .HasColumnType("char");
                entity.Property(e => e.cgrade_desc)
                    .IsRequired()
                    .HasMaxLength(100)
                    .HasColumnType("nvarchar");
                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<course_group>(entity =>
            {
                entity.HasKey(e => e.cgroup_code);

                entity.Property(e => e.cgroup_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.cgroup_desc)
                    .IsRequired()
                    .HasMaxLength(100)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<course_instructor>(entity =>
            {
                entity.HasKey(e => new { e.instructor_code, e.course_code });

                entity.Property(e => e.instructor_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.course_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.confirm_date).HasColumnType("datetime");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.instructor_cost).HasColumnType("money");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<course_train_place>(entity =>
            {
                entity.HasKey(e => new { e.place_code, e.course_code });

                entity.Property(e => e.place_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.course_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.confirm_date).HasColumnType("datetime");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.place_cost).HasColumnType("money");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<course_type>(entity =>
            {
                entity.HasKey(e => new { e.ctype_code, e.cgroup_code });

                entity.Property(e => e.ctype_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.cgroup_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.ctype_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<ini_config>(entity =>
            {
                entity.HasKey(e => new { e.client_code, e.system, e.module, e.cnfig_item });

                entity.Property(e => e.client_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.system).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.module).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.cnfig_item).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.text_value)
                    .IsRequired()
                    .HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<ini_country>(entity =>
            {
                entity.HasKey(e => e.country_code);

                entity.Property(e => e.country_code).ValueGeneratedNever();

                entity.Property(e => e.area_part)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.country_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<ini_district>(entity =>
            {
                entity.HasKey(e => new { e.district_code, e.province_code, e.country_code });

                entity.Property(e => e.district_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.province_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.area_part)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.dist_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<ini_list_zip>(entity =>
            {
                entity.HasKey(e => new { e.province_code, e.country_code, e.district_code, e.subdistrict_code, e.zip_code });

                entity.Property(e => e.province_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.district_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.subdistrict_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.zip_code)
                    .HasMaxLength(5)
                    .HasColumnType("char");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<ini_province>(entity =>
            {
                entity.HasKey(e => new { e.province_code, e.country_code });

                entity.Property(e => e.province_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.area_part)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.pro_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<ini_subdistrict>(entity =>
            {
                entity.HasKey(e => new { e.province_code, e.country_code, e.district_code, e.subdistrict_code });

                entity.Property(e => e.province_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.district_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.subdistrict_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.area_part)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.dist_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<instructor>(entity =>
            {
                entity.HasKey(e => e.instructor_code);

                entity.Property(e => e.instructor_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.confirm_date).HasColumnType("datetime");

                entity.Property(e => e.contactor).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.contactor_detail).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.instructor_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_education>(entity =>
            {
                entity.HasKey(e => new { e.rec_no, e.member_code });

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.colledge_name).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.degree)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.faculty).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_group>(entity =>
            {
                entity.HasKey(e => e.mem_group_code);

                entity.Property(e => e.mem_group_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.mem_group_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_health>(entity =>
            {
                entity.HasKey(e => e.member_code);

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.blood_group)
                    .HasMaxLength(1)
                    .HasColumnType("char");

                entity.Property(e => e.hobby).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.medical_history).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.restrict_food).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.special_skill).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_level>(entity =>
            {
                entity.HasKey(e => e.mlevel_code);

                entity.Property(e => e.mlevel_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.mlevel_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_reward>(entity =>
            {
                entity.HasKey(e => new { e.rec_no, e.member_code });

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.reward_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_site_visit>(entity =>
            {
                entity.HasKey(e => new { e.rec_no, e.member_code });

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.site_visit_desc)
                    .IsRequired()
                    .HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_social>(entity =>
            {
                entity.HasKey(e => new { e.rec_no, e.member_code });

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.social_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_status>(entity =>
            {
                entity.HasKey(e => e.mstatus_code);

                entity.Property(e => e.mstatus_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.mstatus_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_train_record>(entity =>
            {
                entity.HasKey(e => new { e.course_code, e.member_code });

                entity.Property(e => e.course_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.course_grade)
                    .HasMaxLength(1)
                    .HasColumnType("char");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_type>(entity =>
            {
                entity.HasKey(e => new { e.mem_type_code, e.mem_group_code });

                entity.Property(e => e.mem_type_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.mem_group_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.mem_type_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<mem_worklist>(entity =>
            {
                entity.HasKey(e => e.rec_no);

                entity.Property(e => e.rec_no).ValueGeneratedNever();

                entity.Property(e => e.company_name_eng).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.company_name_th).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.office_address).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.position_name_eng).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.position_name_th).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.work_year)
                    .HasMaxLength(4)
                    .HasColumnType("char");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<member>(entity =>
            {
                entity.HasKey(e => e.member_code);

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.birthdate).HasColumnType("datetime");

                entity.Property(e => e.building).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.cid_card)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.cid_card_pic)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.cid_type)
                    .HasMaxLength(1)
                    .HasColumnType("char");

                entity.Property(e => e.district_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.email).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.fax).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.floor)
                    .HasMaxLength(20)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.fname).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.h_no)
                    .HasMaxLength(20)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.lane).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.latitude).HasColumnType("decimal");

                entity.Property(e => e.lname).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.longitude).HasColumnType("decimal");

                entity.Property(e => e.lot_no)
                    .HasMaxLength(20)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.marry_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");

                entity.Property(e => e.mem_group_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.mem_photo)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.mem_type_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.mlevel_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.mobile).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.mstatus_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.nationality)
                    .HasMaxLength(30)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.religion)
                    .HasMaxLength(30)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.parent_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.place_name)
                    .HasMaxLength(50)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.province_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.room)
                    .HasMaxLength(20)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.rowversion)
                    .HasColumnType("timestamp")
                    .ValueGeneratedOnAddOrUpdate();

                entity.Property(e => e.sex)
                    .HasMaxLength(1)
                    .HasColumnType("char");

                entity.Property(e => e.social_app_data).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.street).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.subdistrict_code)
                    .HasMaxLength(8)
                    .HasColumnType("char");

                entity.Property(e => e.tel).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.texta_address)
                    .HasMaxLength(200)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.textb_address)
                    .HasMaxLength(200)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.textc_address)
                    .HasMaxLength(200)
                    .HasColumnType("nvarchar");

                entity.Property(e => e.village).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");

                entity.Property(e => e.zip_code)
                    .HasMaxLength(5)
                    .HasColumnType("char");

                entity.Property(e => e.zone)
                    .HasMaxLength(30)
                    .HasColumnType("nvarchar");
            });

            modelBuilder.Entity<pic_image>(entity =>
            {
                entity.HasKey(e => e.image_code);

                entity.Property(e => e.image_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.image_file).HasColumnType("text");

                entity.Property(e => e.image_name).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.ref_doc_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.ref_doc_type)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<project>(entity =>
            {
                entity.HasKey(e => e.project_code);

                entity.Property(e => e.project_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.budget).HasColumnType("money");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.project_approve_date).HasColumnType("datetime");

                entity.Property(e => e.project_date).HasColumnType("datetime");

                entity.Property(e => e.project_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.project_manager).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<project_course>(entity =>
            {
                entity.HasKey(e => e.course_code);

                entity.Property(e => e.course_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.budget).HasColumnType("money");

                entity.Property(e => e.cgroup_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.charge_head)
                    .HasMaxLength(10)
                    .HasColumnType("char");

                entity.Property(e => e.course_approve_date).HasColumnType("datetime");

                entity.Property(e => e.course_begin).HasColumnType("datetime");

                entity.Property(e => e.course_date).HasColumnType("datetime");

                entity.Property(e => e.course_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.course_end).HasColumnType("datetime");

                entity.Property(e => e.ctype_code)
                    .HasMaxLength(3)
                    .HasColumnType("char");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.project_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.project_manager).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.support_head)
                    .HasMaxLength(10)
                    .HasColumnType("char");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<project_course_register>(entity =>
            {
                entity.HasKey(e => new { e.course_code, e.member_code });

                entity.Property(e => e.course_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<project_daily_checklist>(entity =>
            {
                entity.HasKey(e => new { e.course_date, e.course_code, e.member_code });

                entity.Property(e => e.course_date).HasColumnType("datetime");

                entity.Property(e => e.course_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.member_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<project_sponsor>(entity =>
            {
                entity.HasKey(e => e.spon_code);

                entity.Property(e => e.spon_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.confirm_date).HasColumnType("datetime");

                entity.Property(e => e.contactor).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.contactor_detail).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.spon_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<project_supporter>(entity =>
            {
                entity.HasKey(e => new { e.spon_code, e.project_code });

                entity.Property(e => e.spon_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.project_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.contactor).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.contactor_detail).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.support_budget).HasColumnType("money");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });

            modelBuilder.Entity<train_place>(entity =>
            {
                entity.HasKey(e => e.place_code);

                entity.Property(e => e.place_code)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.confirm_date).HasColumnType("datetime");

                entity.Property(e => e.contactor).HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.contactor_detail).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.id).HasDefaultValueSql("newid()");

                entity.Property(e => e.instructor_desc)
                    .IsRequired()
                    .HasMaxLength(100).HasColumnType("nvarchar");

                entity.Property(e => e.ref_doc)
                    .HasMaxLength(30)
                    .HasColumnType("varchar");

                entity.Property(e => e.x_log).HasMaxLength(500).HasColumnType("nvarchar");

                entity.Property(e => e.x_note).HasMaxLength(50).HasColumnType("nvarchar");

                entity.Property(e => e.x_status)
                    .HasMaxLength(1)
                    .HasColumnType("char");
            });
        }

        public virtual DbSet<course_grade> course_grade { get; set; }
        

        public virtual DbSet<course_group> course_group { get; set; }
        public virtual DbSet<course_instructor> course_instructor { get; set; }
        public virtual DbSet<course_train_place> course_train_place { get; set; }
        public virtual DbSet<course_type> course_type { get; set; }
        public virtual DbSet<ini_config> ini_config { get; set; }
        public virtual DbSet<ini_country> ini_country { get; set; }
        public virtual DbSet<ini_district> ini_district { get; set; }
        public virtual DbSet<ini_list_zip> ini_list_zip { get; set; }
        public virtual DbSet<ini_province> ini_province { get; set; }
        public virtual DbSet<ini_subdistrict> ini_subdistrict { get; set; }
        public virtual DbSet<instructor> instructor { get; set; }
        public virtual DbSet<mem_education> mem_education { get; set; }
        public virtual DbSet<mem_group> mem_group { get; set; }
        public virtual DbSet<mem_health> mem_health { get; set; }
        public virtual DbSet<mem_level> mem_level { get; set; }
        public virtual DbSet<mem_reward> mem_reward { get; set; }
        public virtual DbSet<mem_site_visit> mem_site_visit { get; set; }
        public virtual DbSet<mem_social> mem_social { get; set; }
        public virtual DbSet<mem_status> mem_status { get; set; }
        public virtual DbSet<mem_train_record> mem_train_record { get; set; }
        public virtual DbSet<mem_type> mem_type { get; set; }
        public virtual DbSet<mem_worklist> mem_worklist { get; set; }
        public virtual DbSet<member> member { get; set; }
        public virtual DbSet<pic_image> pic_image { get; set; }
        public virtual DbSet<project> project { get; set; }
        public virtual DbSet<project_course> project_course { get; set; }
        public virtual DbSet<project_course_register> project_course_register { get; set; }
        public virtual DbSet<project_daily_checklist> project_daily_checklist { get; set; }
        public virtual DbSet<project_sponsor> project_sponsor { get; set; }
        public virtual DbSet<project_supporter> project_supporter { get; set; }
        public virtual DbSet<train_place> train_place { get; set; }
    }
}